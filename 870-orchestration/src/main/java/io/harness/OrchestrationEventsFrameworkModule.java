...
import static io.harness.OrchestrationEventsFrameworkConstants.TRIGGER_NODE_EVENT_BATCH_SIZE;
import static io.harness.OrchestrationEventsFrameworkConstants.TRIGGER_NODE_EVENT_CONSUMER;
...
import static io.harness.eventsframework.EventsFrameworkConstants.TRIGGER_NODE_EVENT_MAX_TOPIC_SIZE;
import static io.harness.eventsframework.EventsFrameworkConstants.TRIGGER_NODE_EVENT_TOPIC;
    ...
    if (redisConfig.getRedisUrl().equals("dummyRedisUrl")) {
      ...
      bind(Consumer.class)
          ...
          .toInstance(
              NoOpConsumer.of(EventsFrameworkConstants.DUMMY_TOPIC_NAME, EventsFrameworkConstants.DUMMY_GROUP_NAME));
      bind(Consumer.class)
          .annotatedWith(Names.named(TRIGGER_NODE_EVENT_CONSUMER))
          .toInstance(
              NoOpConsumer.of(EventsFrameworkConstants.DUMMY_TOPIC_NAME, EventsFrameworkConstants.DUMMY_GROUP_NAME));
      bind(Producer.class)
          .annotatedWith(Names.named(TRIGGER_NODE_EVENT_PRODUCER))
      ...
      bind(Producer.class)
          .annotatedWith(Names.named(TRIGGER_NODE_EVENT_PRODUCER))
          .toInstance(RedisProducer.of(TRIGGER_NODE_EVENT_TOPIC, redissonClient, TRIGGER_NODE_EVENT_MAX_TOPIC_SIZE,
              PIPELINE_SERVICE.getServiceId(), redisConfig.getEnvNamespace()));

      bind(Consumer.class)
          .annotatedWith(Names.named(TRIGGER_NODE_EVENT_CONSUMER))
          .toInstance(RedisConsumer.of(TRIGGER_NODE_EVENT_TOPIC, PIPELINE_SERVICE.getServiceId(), redissonClient,
              Duration.ofSeconds(MAX_PROCESSING_TIME_SECONDS), TRIGGER_NODE_EVENT_BATCH_SIZE,
              redisConfig.getEnvNamespace()));
    }
  }
}
