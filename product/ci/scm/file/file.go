...
func parseCrudResponse(ctx context.Context, client *scm.Client, body io.Reader, p pb.Provider, request requestContext, log *zap.SugaredLogger) (commitID, blobID string) {
	...
	case *pb.Provider_BitbucketCloud:
		...
		getLatestCommitRequest := &pb.GetLatestCommitOnFileRequest {
			Slug: request.Slug,
			Branch: request.Branch,
			Provider: &p,
			FilePath: request.FilePath,
		}
		getLatestCommitResponse, err := git.GetLatestCommitOnFile(ctx, getLatestCommitRequest, log)
		if err != nil {
			return "", ""
		}
		return getLatestCommitResponse.CommitId, ""
	default:
		return "", ""
	}
}
